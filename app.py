...
def load_data():
    ...
    return internal, customers, history
...
internal_df, customer_df, history_df = load_data()
...
with col2:
    ...
    # 1. State Border (Bold Black Line)
    folium.GeoJson(
        state_border_gdf,
    ...
    # 2. Customer Network (Fixed jumping issue using random_state=42)
    if show_customers:
    ...
    # 3. Internal Touchpoints (Distinct Colors)
    if show_internal:
    ...
    <div style="position: fixed; bottom: 50px; left: 50px; width: 230px; height: 160px; 
         ...
         <i class="fa fa-circle" style="color:orange; opacity:0.6;"></i> Customer Network
    </div>
    """
